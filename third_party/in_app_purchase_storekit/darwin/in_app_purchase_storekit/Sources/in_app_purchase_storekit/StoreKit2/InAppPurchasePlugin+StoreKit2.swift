  ...
  func purchase(
    ...
    Task { @MainActor in
      do {
        ...
        // Intentionally no-op for win-back offers to maintain compatibility
        // with Xcode/SDK versions that do not expose StoreKit win-back APIs.
    ...
    completion(
      .failure(
        PigeonError(
          ...
          message: "Win back offers are unavailable with this iOS/macOS SDK build.",
          details: "Product ID: \(productId), Offer ID: \(offerId)")))
  }
