...
class TvLogsRepository {
  TvLogsRepository(this._templateLogsRepository, this._localDataSource);

  static const String templateKey = 'tv_log';
  static const String builtInTemplateId = 'builtin:tv_log';
  ...
  Future<List<Map<String, dynamic>>> loadEntries({
    ...
    final resolvedTemplateId = _resolveTemplateId(templateId);
    debugPrint(
      'TVLOGS_RELOAD_QUERY userId=$userId templateKey=$templateKey templateId=$resolvedTemplateId',
    );
    ...
    debugPrint(
      'TVLOGS_LIST_READ_FROM_LOCAL count=${entries.length} userId=$userId',
    );
    return entries;
  }
  ...
  Future<void> addEntry({
    ...
    debugPrint(
      'TVLOGS_SAVE_LOCAL_START day=$day userId=$userId templateId=${_resolveTemplateId(templateId)} data=$data',
    );
    return _templateLogsRepository
        ...
        .then((_) {
          debugPrint('TVLOGS_SAVE_LOCAL_SUCCESS day=$day userId=$userId');
        })
        .catchError((Object error, StackTrace stackTrace) {
          debugPrint('TVLOGS_SAVE_LOCAL_ERROR: $error');
          throw error;
        });
  }
  ...
  Future<void> updateEntry({
    ...
    debugPrint(
      'TVLOGS_SAVE_LOCAL_START day=$day userId=$userId existingId=${existingEntry['id']} data=$data',
    );
    return _templateLogsRepository
        .updateEntry(
          scopeId: scopeId,
        ...
        .then((_) {
          debugPrint(
            'TVLOGS_SAVE_LOCAL_SUCCESS day=$day userId=$userId existingId=${existingEntry['id']}',
          );
        })
        .catchError((Object error, StackTrace stackTrace) {
          debugPrint('TVLOGS_SAVE_LOCAL_ERROR: $error');
...
final tvLogsRepositoryProvider = Provider<TvLogsRepository>((ref) {
  final database = ref.watch(appDatabaseProvider);
  return TvLogsRepository(
