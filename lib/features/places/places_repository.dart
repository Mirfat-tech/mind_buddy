...
class PlacesRepository {
  PlacesRepository(this._templateLogsRepository, this._localDataSource);

  static const String templateKey = 'places';
  static const String builtInTemplateId = 'builtin:places';
  ...
  Future<List<Map<String, dynamic>>> loadEntries({
    ...
    final resolvedTemplateId = _resolveTemplateId(templateId);
    debugPrint(
      'PLACES_RELOAD_QUERY userId=$userId templateKey=$templateKey templateId=$resolvedTemplateId',
    );
    ...
    debugPrint(
      'PLACES_LIST_READ_FROM_LOCAL count=${entries.length} userId=$userId',
    );
    return entries;
  }
  ...
  Future<void> addEntry({
    ...
    debugPrint(
      'PLACES_SAVE_LOCAL_START day=$day userId=$userId templateId=${_resolveTemplateId(templateId)} data=$data',
    );
    return _templateLogsRepository
        ...
        .then((_) {
          debugPrint('PLACES_SAVE_LOCAL_SUCCESS day=$day userId=$userId');
        })
        .catchError((Object error, StackTrace stackTrace) {
          debugPrint('PLACES_SAVE_LOCAL_ERROR: $error');
          throw error;
        });
  }
  ...
  Future<void> updateEntry({
    ...
    debugPrint(
      'PLACES_SAVE_LOCAL_START day=$day userId=$userId existingId=${existingEntry['id']} data=$data',
    );
    return _templateLogsRepository
        .updateEntry(
          scopeId: scopeId,
        ...
        .then((_) {
          debugPrint(
            'PLACES_SAVE_LOCAL_SUCCESS day=$day userId=$userId existingId=${existingEntry['id']}',
          );
        })
        .catchError((Object error, StackTrace stackTrace) {
          debugPrint('PLACES_SAVE_LOCAL_ERROR: $error');
