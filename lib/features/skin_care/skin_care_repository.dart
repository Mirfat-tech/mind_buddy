...
class SkinCareRepository {
  SkinCareRepository(this._templateLogsRepository, this._localDataSource);

  static const String templateKey = 'skin_care';
  static const String builtInTemplateId = 'builtin:skin_care';
  ...
  Future<List<Map<String, dynamic>>> loadEntries({
    ...
    final resolvedTemplateId = _resolveTemplateId(templateId);
    debugPrint(
      'SKINCARE_RELOAD_QUERY userId=$userId templateKey=$templateKey templateId=$resolvedTemplateId',
    );
    ...
    debugPrint(
      'SKINCARE_LIST_READ_FROM_LOCAL count=${entries.length} userId=$userId',
    );
    return entries;
  }
  ...
  Future<void> addEntry({
    ...
    debugPrint(
      'SKINCARE_SAVE_LOCAL_START day=$day userId=$userId templateId=${_resolveTemplateId(templateId)} data=$data',
    );
    return _templateLogsRepository
        ...
        .then((_) {
          debugPrint('SKINCARE_SAVE_LOCAL_SUCCESS day=$day userId=$userId');
        })
        .catchError((Object error, StackTrace stackTrace) {
          debugPrint('SKINCARE_SAVE_LOCAL_ERROR: $error');
          throw error;
        });
  }
  ...
  Future<void> updateEntry({
    ...
    debugPrint(
      'SKINCARE_SAVE_LOCAL_START day=$day userId=$userId existingId=${existingEntry['id']} data=$data',
    );
    return _templateLogsRepository
        .updateEntry(
          scopeId: scopeId,
        ...
        .then((_) {
          debugPrint(
            'SKINCARE_SAVE_LOCAL_SUCCESS day=$day userId=$userId existingId=${existingEntry['id']}',
          );
        })
        .catchError((Object error, StackTrace stackTrace) {
          debugPrint('SKINCARE_SAVE_LOCAL_ERROR: $error');
...
final skinCareRepositoryProvider = Provider<SkinCareRepository>((ref) {
  final database = ref.watch(appDatabaseProvider);
  return SkinCareRepository(
